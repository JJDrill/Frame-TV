...
setInterval(() => {
  ...
  db.Get_App_Config_Setting('Screen Saver Duration').then(function(data){
    ...
    return db.Get_App_Config_Setting('Picture Directory').then(function(data){
      ...
      console.log('Pictures Directory: ', pictures_directory);
    }).then();
    ...
    return db.Get_Slideshow_List().then(function(pic_list){
      ...
      fs.writeFile('./slideshow/slideshow_list.txt', pic_list, function (err) {
        if (err) throw err;
      })
    }).then();
    ...
    if (!DEBUG) {
      ...
      execSync(fehCommand, [pictures_directory]);
    } else {
